export class RegisterLecturaDto{
    private constructor(
        public id: number,
        public fecha: string,
    ...
    static create( object: {[key: string]: any}): [string?, RegisterLecturaDto?]{
       ...
       // if( nombre.length >= 40 ) return ['EL Lectura tiene un nombre demasiado largo', undefined]
        ...
        if( pagado===undefined ) return ['el pagado es obligatorio', undefined]  
        if( !id_tarifa ) return ['el idtarifa es obligatorio', undefined]  
        if( !id_catastro ) return ['el id_catastro es obligatorio', undefined]  
        if( !id_usuario ) return ['el id_usuario es obligatorio', undefined]  
        
        return [undefined, new RegisterLecturaDto(
            id,
            fecha,
            lectura_anterior,
            lectura_actual,
